...
async function claimForToken(token) {
    try {
        ...
        if (coinPoolLeftCount < 250) {
            console.log(`Energy nhỏ hơn 250 token ${token.substring(0, 35)}... Claim token tiếp theo`);
            return true; 
        }
        ...
        if (postResponse.status === 200) {
            ...
            if (getResponse.status === 200) {
                ...
                console.log(`Claim thành công acc ${token.substring(33, 35)}`);
            } else {
...
async function claimAndResetInterval() {
    fs.readFile(tokenFilePath, 'utf8', async (err, data) => {
        ...
        if (currentIndex < tokens.length) {
            ...
            console.log('Đã hoàn tất claim tất cả token, nghỉ 30 GIÂY rồi tiếp tục!');
            currentIndex = 0; 
